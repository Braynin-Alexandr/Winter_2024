...
while True:
    ...
    if number < 0: #если введено отрицательное число, то не учитываем его
        ...
        res = sum(summa_lst) / len(summa_lst)  # ищем среднюю зарплату
...
number=str(int(input("Введите целое число: "))) #вводим целое число и переводим его в строку, чтобы найти с помощью метода count
